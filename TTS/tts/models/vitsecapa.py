## Edited version of vits.py, using a SpeechBrain speaker encoder instead of the default one.
# (originally intended specifically for an ECAPA-TDNN speaker encoder, hence the name "vitsecapa" - but it should work with any similar SpeechBrain model)
# Written for SpeechBrain v0.5.16 => may require some minor code changes to work with the latest version (not tested)
# 
# MK, 2023-2024
...
# New SpeechBrain speaker manager - MK
class ECAPASpeakerManager(SpeakerManager):
    ...
    def init_encoder(self, model_path: str, config_path: str, use_cuda=True) -> None:
        ...
        if use_cuda:
            self.speechbrain_encoder = EncoderClassifier.from_hparams(source=model_path, run_opts={"device":"cuda"})
            print("vitsecapa.py")
            print("using cuda")
        else:
            self.speechbrain_encoder = EncoderClassifier.from_hparams(source=model_path, run_opts={"device":"cpu"})
            print("vitsecapa.py")
            print("using cpu")

        
        device = "cuda" if use_cuda else "cpu"
        self.speechbrain_encoder.to(device)

        #self.encoder = self.speechbrain_encoder
        self.encoder = None
        
        self.encoder_ap = AudioProcessor(**self.encoder_config.audio)

        hyperparams_path = self.encoder_config.model_params["hparams_path"]
        with open(hyperparams_path) as fin:
            self.speechbrain_hparams = load_hyperpyyaml(fin)
    ...
    def compute_embedding_from_clip(self, wav_file: Union[str, List[str]]) -> list:
        ...
        def _compute(wav_file: str):
            ...
            # new code (use Speechbrain for extraction)
            signal,fs = torchaudio.load(wav_file)
            #print("vitsecapa.py - compute_embedding_from_clip")
            #print(fs)
            #print(self.encoder_config)
            if hasattr(self.encoder_config, "audio"):
                print("Resampling audio to %d Hz."%(self.encoder_config.audio["sample_rate"]))
                signal = torchaudio.functional.resample(signal, orig_freq=fs, new_freq=self.encoder_config.audio["sample_rate"])
            else:
                print("Did not resample audio - something is wrong.")
    ...
    def init_from_config(config: "Coqpit", samples: Union[List[List], List[Dict]] = None) -> "ECAPASpeakerManager":
...
class Vitsecapa(BaseTTS):
    """VITS TTS model + ECAPA-TDNN speaker embeddings
    ...
    def __init__(
        ...
        speaker_manager: ECAPASpeakerManager = None, # EDIT MK
    ...
    def init_multispeaker(self, config: Coqpit):
        ...
        if self.args.use_speaker_encoder_as_loss:
            if self.speaker_manager.speechbrain_encoder is None and ( # EDIT MK
            #if self.speaker_manager.encoder is None and (  # EDIT MK
            ...
            #self.speaker_manager.encoder.eval()  # EDIT MK
            print(" > External Speaker Encoder Loaded !!")

            #--------
            # EDIT MK

            # resampling to the SpeechBrain model's required sampling rate

            print("vitsecapa.py - init_multispeaker")
            data_freq = self.config.audio.sample_rate
            encoder_freq = self.speaker_manager.speechbrain_hparams["sample_rate"]
            if (data_freq != encoder_freq):
                self.audio_transform = torchaudio.transforms.Resample(
                    orig_freq=data_freq,
                    new_freq=encoder_freq
                )
                
                print("Speaker encoder requires a different sampling rate -> resampling will be done from %d to %d"%(data_freq,encoder_freq))
            else:
                print("Audio matches the encoder sampling rate -> resampling is not needed")
    ...
    def on_epoch_start(self, trainer):  # pylint: disable=W0613
        ...
        if self.args.use_speaker_encoder_as_loss:
            ...
            self.speaker_manager.speechbrain_encoder = self.speaker_manager.speechbrain_encoder.to(self.device)
    ...
    def forward(  # pylint: disable=dangerous-default-value
        ...
        # EDIT MK
            ...
            wavs_batch_squeezed = wavs_batch.squeeze(1)

            pred_embs = self.speaker_manager.speechbrain_encoder.encode_batch(wavs_batch_squeezed)

                ## this line is like "l2_norm=True" in the original speaker encoder
                ## BUT WE DO NOT WANT TO DO THIS - not for ECAPA-TDNN embeddings!
    ...
    def init_from_config(config: "VitsConfig", samples: Union[List[List], List[Dict]] = None, verbose=True):
        ...
        speaker_manager = ECAPASpeakerManager.init_from_config(config, samples)
        language_manager = LanguageManager.init_from_config(config)

        if config.model_args.speaker_encoder_model_path:
            #print("vitsecapa.py")
            #print(type(speaker_manager))
            #print(speaker_manager)
            speaker_manager.init_encoder(
                config.model_args.speaker_encoder_model_path, config.model_args.speaker_encoder_config_path
            )
        return Vitsecapa(new_config, ap, tokenizer, speaker_manager, language_manager)
